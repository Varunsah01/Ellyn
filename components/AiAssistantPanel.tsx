...
/**
 * Render the AIAssistantPanel component.
 * @param {AIAssistantPanelProps} props - Component props.
 * @returns {unknown} JSX output for AIAssistantPanel.
 * @example
 * <AIAssistantPanel />
 */
export function AIAssistantPanel({
  ...
  const runEnhancement = async (action: EnhanceAction, label: string) => {
    ...
    try {
      const response = await fetch('/api/v1/ai/enhance-draft', {
        method: 'POST',
  ...
  const runToneCustomization = async () => {
    ...
    try {
      const response = await fetch('/api/v1/ai/customize-tone', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          draft: body,
