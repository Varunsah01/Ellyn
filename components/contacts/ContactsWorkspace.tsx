...
/**
 * Render the ContactsWorkspace component.
 * @returns {unknown} JSX output for ContactsWorkspace.
 * @example
 * <ContactsWorkspace />
 */
export default function ContactsWorkspace() {
  ...
  useEffect(() => {
    ...
    if (!focusedContactId || !visibleContacts.some((contact) => contact.id === focusedContactId)) {
      ...
      if (firstVisibleContact) {
        setFocusedContactId(firstVisibleContact.id);
      }
    }
  ...
  const handleMoveFocus = useCallback((direction: 1 | -1) => {
    ...
    if (nextContact) {
      setFocusedContactId(nextContact.id);
    }
  }, [focusedContactId, visibleContacts]);
