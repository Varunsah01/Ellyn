...
import { Mail, FileText, Heart, MessageSquare, Sparkles, Eye, type LucideIcon } from "lucide-react";
...
/**
 * Render the TemplatePicker component.
 * @param {TemplatePickerProps} props - Component props.
 * @returns {unknown} JSX output for TemplatePicker.
 * @example
 * <TemplatePicker />
 */
export function TemplatePicker({ onSelect, open, onOpenChange }: TemplatePickerProps) {
  ...
  const isTemplateCategory = (value: string): value is EmailTemplate["category"] => {
    return (
      value === "cold-outreach" ||
      value === "referral" ||
      value === "follow-up" ||
  ...
  const categoryIcons: Record<EmailTemplate["category"], LucideIcon> = {
    "cold-outreach": Mail,
    "referral": Heart,
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <Tabs
            ...
            onValueChange={(value) => {
              if (value === "all" || isTemplateCategory(value)) {
