...
const TRACKER_PREFS_KEY = "ellyn:tracker-v2-preferences";
const TRACKER_CACHE_KEY = "ellyn:tracker-v2-cache";
...
export default function TrackerPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        setError("Unable to load local tracker contacts.");
      } finally {
        setLoading(false);
      }
    }, 250);
  ...
  const handleExport = (contactsToExport: TrackerContact[], fileTag: string) => {
    ...
    downloadCsv(`ellyn-tracker-${fileTag}-${dateLabel}.csv`, csv);
      ...
      <ConfirmDeleteModal
        ...
        description={
          <>
            ...
            <strong>{selectedContactIds.length}</strong> contacts from your tracker.
